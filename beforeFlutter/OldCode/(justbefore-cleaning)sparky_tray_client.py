...
class SparkyVoiceAssistant:
    ...
    def _speak_text_streaming(self, text):
        """
        Convert text to speech using TRUE STREAMING with ECHO CANCELLATION
        NOW WITH ABORT SUPPORT: Can be interrupted mid-sentence
        """
        try:
            self.is_speaking = True
            ...
            url = f"{TTS_URL}/speak"
            payload = {"text": text, "voice": DEFAULT_VOICE}
            
            if DEBUG_AUDIO:
                print(f"🔍 DEBUG: TTS URL: {url}")
            ...
            if response.status_code == 200:
                ...
                # STREAM WITH ABORT CHECK AND ECHO CANCELLATION BUFFER
                for chunk in response.iter_content(chunk_size=chunk_size):
                    # Check abort flag on every chunk
                    if self.abort_tts:
                        print("🛑 TTS aborted by user")
                        break
                    
                    if chunk:
                        if first_chunk:
                            print("  ✓ First audio chunk playing!")
                            first_chunk = False
                        
                        audio_data = np.frombuffer(chunk, dtype=np.int16)
                        
                        # Store in echo cancellation buffer (convert to float32 to match mic format)
                        if ECHO_CANCEL_ENABLED:
                            # Resample from 24kHz to 16kHz to match mic sample rate
                            # Simple decimation (every 3rd sample for 24k->16k isn't perfect but workable)
                            if TTS_SAMPLE_RATE != SAMPLE_RATE:
                                ratio = TTS_SAMPLE_RATE / SAMPLE_RATE
                                indices = np.arange(0, len(audio_data), ratio).astype(int)
                                resampled = audio_data[indices]
                            ...
                            # Normalize to float32 [-1, 1] to match mic format
                            normalized = resampled.astype(np.float32) / 32767.0
                ...
                if not self.abort_tts:
                    print(f"✓ Playback complete (Powered by {provider.upper()})")
                
                # Clear echo buffer after playback
                with self.echo_cancel_lock:
                    self.tts_playback_buffer.clear()
                ...
                try:
                    ...
                    if 'detail' in error_detail:
                        error_msg = f"{response.status_code}: {error_detail['detail']}"
                except:
                    pass
                print(f"✗ TTS failed: {error_msg}")
                print(f"   Voice requested: '{DEFAULT_VOICE}' | TTS Server: {TTS_URL}")
...
class SparkyTrayApp:
    ...
    def show_about(self, icon=None, item=None):
        ...
        print("\nv3.4 INSTANT GOODBYE:")
        print("  ✓ Goodbye plays IMMEDIATELY when exit word detected")
        print("  ✓ All cleanup (stream stopping, etc.) happens AFTER")
        print("  ✓ No more 10+ second wait!")
        print("="*60 + "\n")
