#!/usr/bin/env python3
"""
Sparky Voice-AI System Tray Client v4.3.0
...
class ChatWindow:
    ...
    def _setup_ui(self):
        ...
        # Top toolbar
        toolbar = tk.Frame(self.window, relief=tk.RAISED, borderwidth=1)
        ...
        self.mode_label = tk.Label(toolbar, text="", font=("Segoe UI", 9), fg="blue")
        ...
        # Input text box
        self.input_text = tk.Text(input_frame, height=3, font=("Segoe UI", 10), wrap=tk.WORD)
        ...
        self.typing_label = tk.Label(self.window, text="", font=("Segoe UI", 9, "italic"), fg="gray")
        
    def _setup_styles(self):
        """Setup text tags for styling"""
        # User message style (right-aligned, blue)
        self.chat_text.tag_config("user_msg", background="#4A90E2", foreground="white", 
                                   lmargin1=200, lmargin2=200, rmargin=10, 
                                   spacing1=5, spacing3=5, wrap=tk.WORD)
        
        # AI message style (left-aligned, gray)
        self.chat_text.tag_config("ai_msg", background="#E8E8E8", foreground="black",
                                   lmargin1=10, lmargin2=10, rmargin=200,
                                   spacing1=5, spacing3=5, wrap=tk.WORD)
        
        # Timestamp style
        self.chat_text.tag_config("timestamp", font=("Segoe UI", 8), foreground="gray")
    ...
    def _display_message(self, role: str, content: str, timestamp: datetime = None):
        ...
        print(f"🖼️ _display_message called: role={role}, content_len={len(content)}, messages_count={len(self.messages)}")
        
        self.chat_text.config(state=tk.NORMAL)
        print(f"   Chat text state: NORMAL, current content length: {len(self.chat_text.get('1.0', tk.END))}")
        ...
        print(f"   ✓ Message displayed, new content length: {len(self.chat_text.get('1.0', tk.END))}")
        self.chat_text.see(tk.END)
        
        # DEBUG: Schedule verification after a delay
        self.window.after(100, self._debug_verify_display)
    ...
    def _update_last_message(self, content: str):
        ...
        # We search backwards for the last "ai_msg" tag
        try:
            # Get all text
            text_content = self.chat_text.get("1.0", tk.END)
            
            # Find the last occurrence of the AI icon
            last_ai_pos = text_content.rfind("🤖")
            if last_ai_pos != -1:
                # Convert to Tkinter index
                line_start = text_content[:last_ai_pos].count('\n') + 1
                
                # Find where the actual message starts (after timestamp line)
                timestamp_end = f"{line_start}.0 lineend"
                msg_start = f"{line_start + 1}.0"
                
                # Find the end of this message (two newlines or end of text)
                msg_end_search = self.chat_text.search("\n\n", msg_start, tk.END)
                if msg_end_search:
                    msg_end = msg_end_search
                else:
                    msg_end = tk.END
                
                # Delete old message content and insert new
                self.chat_text.delete(msg_start, msg_end)
                self.chat_text.insert(msg_start, f"{content}\n\n", "ai_msg")
    ...
    def _debug_verify_display(self):
        """Debug: Verify display state"""
        content = self.chat_text.get('1.0', tk.END)
        print(f"🔍 Display verification: messages_list={len(self.messages)}, display_length={len(content)}, display_empty={len(content.strip()) == 0}")
        if len(content.strip()) == 0 and len(self.messages) > 0:
            print(f"   ⚠️⚠️⚠️ DISPLAY IS EMPTY BUT MESSAGES LIST HAS {len(self.messages)} MESSAGES!")
    ...
    def send_message(self):
        ...
        print(f"\n📤 send_message called: text='{text[:50]}...', messages_count_before={len(self.messages)}")
        
        # Clear input
        self.input_text.delete("1.0", tk.END)
        print(f"   Input cleared")
        
        # Add user message to display
        user_msg = ChatMessage("user", text)
        self.messages.append(user_msg)
        print(f"   User message added to messages list, new count={len(self.messages)}")
        ...
        if not self.ws_connected:
            print(f"   ⚠️ Not connected, attempting to reconnect...")
    ...
    async def _websocket_handler(self):
        ...
        try:
            async with websockets.connect(ORCH_WS_URL, ping_interval=20, ping_timeout=10) as ws:
                ...
                print("✅ Text chat WebSocket connected")
                ...
                print("🔧 Creating send and receive tasks...")
                ...
                print(f"   Recv task created: {recv_task}")
                
                # Wait for BOTH tasks (they should run until connection closes)
                # Use gather() instead of wait() to keep both running
                print("🔧 Starting gather() on both tasks...")
                try:
                    await asyncio.gather(send_task, recv_task, return_exceptions=True)
                    print("🔧 gather() completed normally")
                except Exception as e:
    ...
    async def _send_handler(self, ws):
        ...
        print("📤 Send handler COROUTINE ENTERED")
        print("📤 Send handler started")
        print(f"   ws_connected={self.ws_connected}, _closing={self._closing}")
        
        while not self._closing and self.ws_connected:
            try:
                # CRITICAL FIX: Use get_nowait() instead of blocking get()
                # Blocking queue.get(timeout=0.5) blocks the ENTIRE asyncio event loop!
                try:
    ...
    async def _receive_handler(self, ws):
        ...
        print("📥 Receive handler COROUTINE ENTERED")
        print("📥 Receive handler started")
        print(f"   ws_connected={self.ws_connected}, _closing={self._closing}")
        ...
        print(f"   Loop condition: ws_connected={self.ws_connected}, _closing={self._closing}")
        
        while not self._closing and self.ws_connected:
            try:
                print("📥 Waiting for message...")
                msg = await ws.recv()
                print(f"📥 GOT MESSAGE: {type(msg)}, length={len(msg) if isinstance(msg, str) else 'N/A'}")
                ...
                if isinstance(msg, str):
                    ...
                    if msg_type == "text_token":
                        ...
                        print(f"🔤 Token: '{token}' (total length: {len(current_response)})")
    ...
    def _handle_responses(self):
        ...
        while self._response_handler_active and not self._closing:
            try:
                ...
                print(f"🖼️ UI update: type={msg_type}, length={len(str(content))}")
                
                if msg_type == "token":
                    # Update UI with streaming token
                    if not streaming_started:
                        # First token - create new message
                        print("  → Creating new AI message")
                    ...
                    if self.messages:
                        ...
                        print("  → No streaming, creating message")
                        ai_msg = ChatMessage("assistant", content)
                        self.messages.append(ai_msg)
                        self.window.after(0, lambda t=content: self._display_message("assistant", t))
        ...
        print("🖥️ Response handler thread stopped")
